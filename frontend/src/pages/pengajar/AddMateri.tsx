...
const AddMateri: React.FC = () => {
  ...
  useEffect(() => {
    if (pertemuanId) {
      fetchMeetingAndMateri();
    } else {
      setLoading(false);
    }
  }, [pertemuanId]);

  const fetchMeetingAndMateri = async () => {
    setLoading(true);
    try {
      // 1. Get meeting detail from all meetings list
  ...
  const topicName = meeting ? `P${meeting.urutan} ${meeting.topik || 'Tanpa Topik'}` : 'Materi';
      ...
      <div className="flex items-center gap-6 mb-8">
        ...
        <div>
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
             Kelola video, file sub-materi, refleksi, dan latihan soal pilihan ganda
          </p>
        </div>
      </div>
